/* =======================================================
   CrossSlipPro — script.js (minimal UI + robust analyze)
   ======================================================= */
...
function parseSlip(text){
  ...
  const market=/\bMoneyline\b|ML\b/i.test(text)?"Moneyline":/\bSpread\b|pts|point|handicap/i.test(text)?"Spread":/\bTotal\b|Over|Under/i.test(text)?"Total/Prop":(props.length?"Prop/Total":"Unknown");
...
(function restoreFromHash(){
  if(location.hash.length>1){
    ...
    if(data){
      $("#book")   && ($("#book").value   = state.book   = data.book   || "Unknown");
      $("#league") && ($("#league").value = state.league = data.league || "Unknown");
      $("#market") && ($("#market").value = state.market = data.market || "Unknown");
      $("#title")  && ($("#title").value  = state.title  = data.title  || "");
      state.teams=data.teams||[]; state.odds=data.odds||[]; state.competitor=data.competitor||[];
      renderLists(); analyze();
    }
  }
})();
...
async function generateShareImage(){
  ...
  [gen,cpy,dl].forEach(b=>b && (b.disabled=true));
  try{
...
$("#analyzeBtn")?.addEventListener("click", async ()=>{
  ...
  try{
    ...
    if(!state.odds.length){ out.textContent="OCR finished. No odds detected — add manually if needed."; return; }
    analyze();
    out.textContent="Done ✔  Scroll to Summary.";
    scrollToSummary();
  }catch(e){
    out.textContent="OCR failed. You can fill fields manually.";
    console.error(e);
  }
});
